...
import faiss # [pip install faiss-cpu] / [run conda install -c conda-forge pytorch faiss-cpu]
import pickle 
import urllib
import requests
import numpy as np
...
with open('./back_end/python/vctr_idx/faiss_index10mill.pickle','rb') as infile:
# with open('./back_end/python/vctr_idx/faiss_index5690k.pickle','rb') as infile:
# with open('./back_end/python/vctr_idx/faiss_index_800k_ish.pickle','rb') as infile:
# with open('./back_end/python/vctr_idx/faiss_index.pickle','rb') as infile:
    index = faiss.deserialize_index(pickle.load(infile))
...
def vector_search(query, model, index, num_results=5):
...
def search(request):
    if request.method == "GET":
        ...
        search_term = request.GET["query"].strip()
        ...
        # ================VECTOR SEARCH================
            ...
            # I_list, D, I, index = None, None, None, None 
            D, I = None, None 
